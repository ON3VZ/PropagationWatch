...
/**
 * Fetch Kp, SFI and storm scale from NOAA.
...
export async function fetchNOAA() {
  try {
    ...
    Object.assign(state.propagation, { kp, sfi, gScale: g, fetchedAt: cache.fetchedAt, stale: false });
...
function useCache() {
  ...
  if (!cache) {
    // Conservative defaults — pessimistic but no false positives
